...
export default function BatchList({ orderId }) {
  ...
  const handleDelivered = async (batchIndex) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Batch "${updatedBatch.batchName}" delivered!`);
  ...
  const handleDelete = async (batchIndex) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBatches((data.batches || []).filter((batch) => batch.status === "pending"));
      } else {
          ...
          {batches.map((batch, bIdx) => (
            <div key={bIdx} className="border border-gray-200 rounded-lg p-4 shadow-sm">
              <div className="flex justify-between items-center mb-3">
                <h4 className="font-medium text-gray-700">{batch.batchName || `Batch ${bIdx + 1}`}</h4>
                            ...
                            <div className="space-y-1">
                              {row.extraInputs?.map((input, idx) => (
                                <input
                                  ...
                                  onChange={(e) => handleExtraInputChange(bIdx, rIdx, idx, e.target.value)}
                                />
                              ))}
                              <button
                                type="button"
