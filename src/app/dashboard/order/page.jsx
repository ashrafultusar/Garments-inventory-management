...
const Orders = () => {
  ...
  // URL change hole order load koro
  useEffect(() => {
    if (orderIdFromUrl) {
  ...
  const handleOrderClick = (id) => {
    router.push(`/dashboard/order?orderId=${id}`, { scroll: false });
  };
  ...
  const handleDelete = async () => {
    ...
    setShowConfirmModal(false);
    if (selectedOrder && selectedOrder._id === orderToDelete) {
      // current open order delete hoile modal off + URL reset
      setSelectedOrder(null);
  ...
  const handleCustomApply = () => {
    ...
    // ekhane filter apply logic thakbe jodi lagay
  };
