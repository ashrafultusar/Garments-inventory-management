  ...
  useEffect(() => {
    const fetchLedgerData = async () => {
      try {
        ...
        if (result.success) {
          const { dyeing, billings, payments } = result.data;

          const combined = [
            ...billings.map(b => ({
              date: b.createdAt,
              provider: "DYEING BILL",
        ...
        toast.error("Failed to load dyeing ledger");
        setData(prev => ({ ...prev, loading: false }));
      }
    };

    if (dyeingId) fetchLedgerData();
  }, [dyeingId]);
